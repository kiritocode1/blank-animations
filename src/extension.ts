...
export function activate(context: vscode.ExtensionContext) {
	const installMethod = forVSCode ? (vscode.workspace.getConfiguration("animations").get("Install-Method") as InstallMethod) : InstallMethod.apcCustomizeUI; //Get the install method
	...
	if (forVSCode && (context.globalState.get("firstTime") === undefined || context.globalState.get("firstTime") === true)) {
	...
	const autoInstall = vscode.workspace.getConfiguration("animations").get("Auto-Install") as boolean; //Get the auto install setting
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.installAnimations", () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.changeInstallMethod", () => {
			if (forVSCode) {
				installManager.showInstallMethodPicker();
			} else {
				vscode.window.showErrorMessage("This command is not available in VSCodium");
			}
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.getScriptPath", () => {
			vscode.env.clipboard.writeText(installManager.getPath());
			vscode.window.showInformationMessage(`Animations Script Path Copied to Clipboard! (${installManager.getPath()})`);
		}),
	);
	 ...
	 */
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.disableAnimations", () => {
			vscode.workspace.getConfiguration("animations").update("Enabled", false, vscode.ConfigurationTarget.Global);
	 ...
	 */
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.enableAnimations", () => {
			vscode.workspace.getConfiguration("animations").update("Enabled", true, vscode.ConfigurationTarget.Global);
	 ...
	 */
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.openAnimationSettings", () => {
			vscode.commands
	 ...
	 */
	context.subscriptions.push(
		vscode.commands.registerCommand("Blank-Animations.openCustomCSS", () => {
			vscode.workspace
				.openTextDocument(vscode.Uri.file(context.extensionPath + "/dist/css/Custom-Animations.css")) //Open the custom animations css file
				.then((doc) => {
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeTextDocument((e) => {
			...
			if (e.document.uri.fsPath === context.extensionPath + "/dist/css/Custom-Animations.css" && vscode.workspace.getConfiguration("animations").get("Custom-CSS")) {
				generateCSS(context); //Generate the css file
			}
		}),
	);
