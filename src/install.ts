...
export class InstallationManager {
		...
		if (forVSCode) {
			...
			vscode.workspace.onDidChangeConfiguration((event) => {
				...
				if (event.affectsConfiguration("animations.Install-Method")) {
					const newInstallMethod = vscode.workspace.getConfiguration("animations").get("Install-Method") as InstallMethod; //Get the new install method from the config

					vscode.window.showInformationMessage(`Blank Animations: Install Method now ${newInstallMethod}`);

					//Remove the old install method from the config
					this.removeFromConfig().then(() => {
						// Uninstall the old install method
						vscode.commands.executeCommand(installMethodDetails[this.installMethod].uninstallCommand);

						if (vscode.extensions.getExtension(installMethodDetails[this.installMethod].extensionID)) {
	...
	public showInstallMethodPicker() {
		...
		vscode.window
			...
			.then((value) => {
				if (value?.label) {
					vscode.workspace.getConfiguration("animations").update("Install-Method", value.label, true); //Update the install method in the settings
				}
			});
	}
	...
	public checkForInstallMethod() {
		if (
			vscode.extensions.getExtension(installMethodDetails[InstallMethod.customCSSAndJS].extensionID) &&
			vscode.extensions.getExtension(installMethodDetails[InstallMethod.apcCustomizeUI].extensionID)
		) {
			vscode.window
				.showErrorMessage(`Blank Animations: Both Custom CSS and JS and Apc Customize UI++ are installed`, `Use ${InstallMethod.customCSSAndJS}`, `Use ${InstallMethod.apcCustomizeUI}`)
				.then((value) => {
					if (value === `Use ${InstallMethod.customCSSAndJS}`) {
						this.installMethod = InstallMethod.customCSSAndJS;
						this.path = this.generatePath();
						vscode.workspace.getConfiguration("animations").update("Install-Method", InstallMethod.customCSSAndJS, true);
						return;
					} else if (value === `Use ${InstallMethod.apcCustomizeUI}`) {
						this.installMethod = InstallMethod.apcCustomizeUI;
						this.path = this.generatePath();
						vscode.workspace.getConfiguration("animations").update("Install-Method", InstallMethod.apcCustomizeUI, true);
						return;
					}
				});
		}
		if (vscode.extensions.getExtension(installMethodDetails[this.installMethod].extensionID)) return;

		for (const installMethod of Object.values(InstallMethod)) {
			if (vscode.extensions.getExtension(installMethodDetails[installMethod].extensionID)) {
				vscode.window.showInformationMessage(`Blank Animations: Install Method is ${installMethod} given it is already installed`);
				this.installMethod = installMethod;
				this.path = this.generatePath();
				vscode.workspace.getConfiguration("animations").update("Install-Method", installMethod, true);
			}
		}
	}
	...
	public verifyInstallMethod() {
		...
		if (!vscode.extensions.getExtension(installDetails.extensionID)) {
			...
			vscode.window
				.showErrorMessage(
					`Blank Animations: Please install ${installDetails.extensionName} for animations to work`,
				...
				.then((value) => {
					...
					if (value === `Install ${installDetails.extensionName}`) {
						...
						vscode.commands.executeCommand("workbench.extensions.installExtension", installDetails.extensionID).then(() => {
							vscode.commands.executeCommand("workbench.action.reloadWindow"); //Reload the window
						});
					} else if (value === "Change Install Method") {
						vscode.commands.executeCommand("Blank-Animations.changeInstallMethod");
					}
				});
			return false;
		}
	...
	public install(auto = false) {
		...
		if (auto) {
			if (this.addToConfigNeeded()) {
				vscode.window
					.showInformationMessage(`Blank Animations: Install Required, installation method is ${installMethodDetails[this.installMethod].extensionName}, window will reload`, "Install Now")
					.then((value) => {
						//If the user clicked the install button
						if (value === "Install Now") {
							this.addToConfig(auto).then((added) => {
								//Run the install command for the install method
								vscode.commands.executeCommand(installMethodDetails[this.installMethod].installCommand);
								if (this.installMethod === InstallMethod.customCSSAndJS) {
									vscode.commands.executeCommand("workbench.action.reloadWindow"); //Reload the window
								}
							});
	...
	private addToConfig(auto = false): Thenable<boolean> {
		...
		let customImports = config.get<string[]>(installMethodDetails[this.installMethod].importSetting); //Get the current list of imports

		if (auto) {
			if (customImports && customImports.length > 0) {
				const regex = /kiritocode1\.blank-animations-\d+\.\d+\.\d+/; //Regex to match the version number in the extension id
		...
		if (customImports) customImports = this.removeOldConfigPaths(this.path, customImports); //Remove any old paths from the list
	...
	private removeOldConfigPaths(currentPath: string, configPaths: string[]) {
		...
		const endOfPathDev = "Blank-Animations/dist/updateHandler.js";
		...
		for (let i = 0; i < configPaths.length; i++) {
			...
			if (
				(path.substring(path.length - endOfPath.length) === endOfPath || //If path ends with /dist/updateHandler.js
					path.substring(path.length - endOfPathDev.length) === endOfPathDev) && //If path ends with Blank-Animations/dist/updateHandler.js
				(path.includes("Blank-Animations") || path.includes("kiritocode1.blank-animations")) //If path contains reference to Blank-Animations
			)
				continue;
	...
	private removeFromConfig(): Thenable<void> {
		const config = vscode.workspace.getConfiguration();
		const customCssImports = config.get<string[]>(installMethodDetails[this.installMethod].importSetting); //Get the current list of imports
		if (customCssImports) this.removeOldConfigPaths(this.path, customCssImports);
		...
		if (customCssImports && customCssImports.includes(this.path)) {
			...
			//Update the list of imports
			return config.update(installMethodDetails[this.installMethod].importSetting, customCssImports, vscode.ConfigurationTarget.Global); //Returns the promise of the update
		}
